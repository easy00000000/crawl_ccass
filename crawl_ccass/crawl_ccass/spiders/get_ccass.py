...
class CCASS_Selenium_Spider(Spider):
    ...
    def start_requests(self):    
        ...
        if (crawl_type == '1'):
            ...
            print '[%s] wrong parameters input' % datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            pass
        ...
        for stockid in stocklist:           
            ...
            print '[%s] start to crawl %s.hk' % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), stockid)
        ...
        try:
            ...
            for sdate in sdates:                                   
                try:
                    ...
                    print '[%s] fail to crawl %s at %s' % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), stockid, sdate)
                    break
        except:
            # self.logger.info('fail to crawl the web page of the above stock')
            print '[%s] fail to crawl the web page of the above stock' % datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def parse_ccass_2(self, response):
        try:
            ...
            if (self.get_tuple_value(stockid, sdate) in self.existing_data):
                ...
                print '[%s] parse %s on the date %s' % (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), stockid, sdate)
            ...
            print '[%s] fail to parse data' % datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def read_existingdata(self):
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT StockID, Date FROM stockid_date_index GROUP BY StockID, Date')
        results = cursor.fetchall()
        conn.close()
        # Load Tuple to List
        st_data = []
